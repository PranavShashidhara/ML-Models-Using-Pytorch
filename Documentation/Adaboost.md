...
**How the stump chooses its split:**
The stump evaluates every possible threshold and calculates the **Gini Index** for each split:

```
Gini = 1 - Σ(pᵢ²)
```

Where pᵢ is the proportion of each class in a node.

- Gini = 0 means perfect purity (all samples in a node are the same class)
- Gini = 0.5 means maximum impurity (50/50 split between classes)

The stump picks the threshold that minimizes the weighted average Gini index across both resulting branches.

**Example:** Your first stump evaluates "x > 3?", "x > 4?", "x > 5?", etc., and finds that "x > 5.5" gives the lowest Gini index, so it uses that split.
...
1. **First doctor** evaluates all possible symptoms and their thresholds
   - Calculates Gini index for each: "Temperature > 99°F?", "Temperature > 100°F?", etc.
   - Chooses the split with lowest impurity: "Temperature > 100°F?"
   - Catches most flu cases but misses some edge cases
   
2. **Second doctor** focuses specifically on those missed cases (now with higher weights)
   - Again tests all thresholds, finds the best split for the weighted data
   - "Cough lasting > 3 days?"
   
3. **Third doctor** again focuses on remaining mistakes
   - Uses Gini to find optimal split among remaining hard cases
   - "Fatigue level > 7/10?"

Each doctor uses the Gini index to find their best simple test, but they work on increasingly difficult cases.
